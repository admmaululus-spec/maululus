...
export default function TabProyek({ premiumProjects, activeProject, setActiveMenu }: any) {
  // Menggabungkan props agar semua proyek aktif bisa ditampilkan
      ...
      {projectsToDisplay.length > 0 ? (
        <div className="space-y-8">
          {projectsToDisplay.map((project: any, index: number) => {
            ...
            return (
              <div key={project.id || index} className="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm">
                 {/* Detail Proyek */}
                 <div className="border border-slate-100 rounded-2xl p-6 md:p-8 relative overflow-hidden bg-linear-to-br from-slate-50 to-white">
                   ...
                   <h3 className="text-xl font-black text-slate-900 mb-2 max-w-[85%]">{project.judul || `Proyek Skripsi #${index + 1}`}</h3>
                   <p className="text-sm text-slate-500 mb-8 font-medium">
                     Paket <span className="text-indigo-600 font-bold">{project.paket}</span> • Dikerjakan oleh <span className="text-indigo-600 font-bold">{project.expert}</span>
                   </p>
                   ...
                   <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm mb-10 relative overflow-hidden">
                      ...
                      <div className="flex justify-between items-center mb-4">
                        ...
                        <span className={`text-2xl font-black ${project.progress === 100 ? 'text-emerald-500' : 'text-blue-600'}`}>{project.progress || 0}%</span>
                      </div>
                      <div className="w-full bg-slate-100 rounded-full h-4 overflow-hidden shadow-inner">
                         <div className={`h-4 rounded-full transition-all duration-1000 ease-out relative ${project.progress === 100 ? 'bg-emerald-500' : 'bg-blue-600'}`} style={{ width: `${project.progress || 0}%` }}>
        ...
        <div className="py-20 bg-white border border-slate-200 border-dashed rounded-3xl text-center flex flex-col items-center">
          ...
          <button 
            ...
            className="bg-amber-500 text-amber-950 px-6 py-3 rounded-xl text-sm font-bold hover:bg-amber-400 shadow-md transition-all active:scale-95"
          >
            Lihat Pilihan Paket
          </button>
        </div>
