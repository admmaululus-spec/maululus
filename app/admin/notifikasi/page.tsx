        ...
        <div className="bg-white rounded-3xl p-8 border border-slate-200 shadow-sm max-w-2xl">
          ...
          <form onSubmit={handleKirimBroadcast} className="space-y-5">
            <div>
              ...
              <select className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none text-slate-800 font-medium" onChange={(e) => setFormBroadcast({...formBroadcast, isGlobal: e.target.value === 'all'})}>
                <option value="all">Semua Pengguna (Broadcast)</option>
                <option value="specific">Spesifik User (Berdasarkan ID)</option>
              </select>
            </div>
            
            {/* PERBAIKAN: Menghapus baris komentar di dalam blok boolean ini */}
            {!formBroadcast.isGlobal && (
               <input type="text" placeholder="Masukkan ID User (UUID)" className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none text-slate-800 placeholder-slate-400" onChange={(e) => setFormBroadcast({...formBroadcast, userId: e.target.value})} required/>
            )}

            <div className="grid grid-cols-4 gap-4">
              <div className="col-span-1">
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Ikon</label>
                <input type="text" value={formBroadcast.icon} onChange={(e) => setFormBroadcast({...formBroadcast, icon: e.target.value})} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none text-center text-xl text-slate-800" />
              </div>
              <div className="col-span-3">
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Judul Pesan</label>
                <input type="text" value={formBroadcast.title} onChange={(e) => setFormBroadcast({...formBroadcast, title: e.target.value})} placeholder="Contoh: Promo Flash Sale!" className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none text-slate-800 placeholder-slate-400 font-medium" required/>
              </div>
            </div>
            <div>
              <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Isi Pesan</label>
              <textarea value={formBroadcast.message} onChange={(e) => setFormBroadcast({...formBroadcast, message: e.target.value})} placeholder="Tulis deskripsi notifikasi..." className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none h-32 resize-none text-slate-800 placeholder-slate-400" required></textarea>
            </div>
            <button type="submit" disabled={loading} className="w-full bg-blue-600 text-white font-bold py-3.5 rounded-xl hover:bg-blue-700 transition-colors shadow-md">{loading ? 'Mengirim...' : 'Kirim Sekarang 🚀'}</button>
          </form>
        </div>
      )}
      ...
      {activeTab === 'rules' && (
        <div className="bg-white rounded-3xl p-8 border border-slate-200 shadow-sm">
          <div className="flex justify-between items-center mb-6">
             ...
             <button className="bg-emerald-50 text-emerald-600 font-bold px-4 py-2 rounded-xl text-sm hover:bg-emerald-100 transition-colors">+ Tambah Rule Baru</button>
          </div>
          <table className="w-full text-left text-sm text-slate-600">
             ...
             <tbody>
               {rules && rules.length > 0 ? rules.map((rule) => (
                 ...
                 <tr><td colSpan={4} className="p-8 text-center text-slate-400">Belum ada rule yang dibuat.</td></tr>
               )}
             </tbody>
          </table>
        </div>
